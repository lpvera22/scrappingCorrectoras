...
path = '/media/laura/dados/Projects/Work/searchKeyword/out/img'
...
def modifyJsonAllUpload(newFile):
    with open('src/configAPi.json', 'w') as outfile:
        json.dump(newFile, outfile)
    blob = bucket.blob('json/calApiFont.json')
    with open('src/configAPi.json', 'rb') as f:
        blob.upload_from_file(f)


def modifyJsonFileUpload(newUrl):
    with open('src/configAPi.json') as f:
        data = json.load(f)
